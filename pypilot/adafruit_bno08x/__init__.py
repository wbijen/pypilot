...
class BNO08X:
    ...
    def calibration_status(self) -> int:
        """Get the magnetometer calibration accuracy (0-3) from the last packet.

        Patched for pypilot: returns the cached value directly instead of
        issuing a _send_me_command I2C transaction, which corrupts the SHTP
        sequence-number state when interleaved with _process_available_packets().
        """
    ...
    def _handle_packet(self, packet: Packet) -> None:
        ...
        except Exception:  # bad/short packet during sensor init; skip and retry
            pass
